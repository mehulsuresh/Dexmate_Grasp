...
class RewardsCfg:
    ...
    # Finger enclosure reward (added)
    finger_enclosure = RewTerm(
        func=mdp.finger_enclosure_reward,
        params={
            "object_cfg": SceneEntityCfg("object"),
            "ee_frame_cfg": SceneEntityCfg("ee_frame"),  # Ensure this matches your end-effector frame
            "enclosure_threshold": 0.05,  # Adjust based on your needs
        },
        weight=15.0  # Adjust weight to balance with other rewards
    )

    # Grasp stability reward (NEW)
    grasp_stability = RewTerm(
        func=mdp.grasp_stability_reward,  # or just `grasp_stability_reward` if imported directly
        params={"object_cfg": SceneEntityCfg("object")},
        weight=2.0  # Tune this based on training behavior
    )
